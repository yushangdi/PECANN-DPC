 	...
 	try {
	    ...
	    desc.add_options()("output_file",
			...
			desc.add_options()("density_cutoff",
				       po::value<float>(&density_cutoff)->default_value(0),
				       "Density below which points are treated as noise");
	    desc.add_options()("dist_cutoff",
				       po::value<float>(&dist_cutoff)->default_value(std::numeric_limits<float>::max()),
				       "Distance below which points are sorted into the same cluster");
			desc.add_options()("bruteforce",
				       po::value<bool>(&bruteforce)->default_value(false),
