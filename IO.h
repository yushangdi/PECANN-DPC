...
inline std::pair<std::vector<char*>, size_t> stringToWords(std::vector<char> &Str) {
  size_t n = Str.size();
  std::vector<char*> SA;
...
inline void load_text_file(const std::string& text_file, float*& data,
  ...
  std::cout << "allocating aligned memory of " << allocSize
                << " bytes... " << std::flush;
  alloc_aligned(((void**) &data), allocSize, 8 * sizeof(float));
  std::cout << "done. Copying data to mem_aligned buffer..."
                << std::flush;
  for (size_t i = 0; i < npts; i++) {
    for (size_t d = 0; d < dim; d++){
      *(data + i * rounded_dim + d) = atof(words[i * dim + d]);
    }
    memset(data + i * rounded_dim + dim, 0, (rounded_dim - dim) * sizeof(float));
  }
