...
template <class vertex> struct union_find {
  parlay::sequence<std::atomic<vertex>> parents;

  bool is_root(vertex u) { return parents[u] < 0; }

  // initialize n elements all as roots
  union_find(size_t n)
      : parents(
            parlay::tabulate<std::atomic<vertex>>(n, [](long) { return -1; })) {
  }
  ...
  // when no cycles are created (e.g. only link from lower density to higher
  // density. Does not use ranks.
  void link(vertex u, vertex v) { parents[u] = v; }
};

using namespace std;

template <typename IntType> struct ParUF {

  IntType *parents;
  pair<IntType, IntType>
      *hooks; // the edge that merged comp idx with a higher idx comp
  IntType m_n;
  double *values;
  bool store_value;
  ...
  ParUF(IntType n, bool t_store_value = false) {
    ...
    hooks =
        (pair<IntType, IntType> *)malloc(sizeof(pair<IntType, IntType>) * n);

    parlay::parallel_for(0, n, [&](IntType i) {
      parents[i] = std::numeric_limits<IntType>::max();
      hooks[i] = make_pair(std::numeric_limits<IntType>::max(),
  ...
  inline IntType find(IntType i) {
    IntType j = i;
    if (parents[j] == std::numeric_limits<IntType>::max())
      return j;
    do
      j = parents[j];
  ...
  IntType link(IntType u, IntType v) {
    ...
    while (1) {
      ...
      if (hooks[u].first == std::numeric_limits<IntType>::max() &&
          __sync_bool_compare_and_swap(
              &hooks[u].first, std::numeric_limits<IntType>::max(), c_from)) {
        parents[u] = v;
        hooks[u].second = c_to;
        break;
      }
